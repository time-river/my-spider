...
class MovieSpider(Spider):
    ...
    def informationParse(self, response):
        ...
        # 连续剧内容 
        season_1 = selector.xpath('//*[@id="season"]/option[@selected="selected"]/text()').extract()
        seasion_2 = selector.re(r'<span[\s]+class="pl">季数[\W]+</span>(.*?)<br')
        if season_1: # 两种模式
            information['season'] = seasion_1
        else:
            information['season'] = seasion_2
